...
impl User {
    ...
    pub async fn fetch_user(pool: &Pool, id: ID<'_>) -> Result<User, sqlx::Error> {
        match id {
    ...
    pub async fn get_user(pool: &Pool, email: &str, password: &str) -> Result<User, sqlx::Error> {
        let user = Self::fetch_user(pool, ID::Email(email)).await?;
        Argon2::default().verify_password(password.as_bytes(), &PasswordHash::new(&user.password).map_err(|_| sqlx::Error::WorkerCrashed)?).map_err(|_| {
            sqlx::Error::WorkerCrashed
        })?;
        Ok(user)
    }

    pub async fn create_user(pool: &Pool, user: User) -> Result<User, sqlx::Error> {
        ...
        let password_hash = argon2.hash_password(user.password.as_bytes(), &salt).map_err(|_| {
            sqlx::Error::WorkerCrashed
        })?.to_string();
    ...
    pub async fn delete_user(pool: &Pool, email:&str) -> Result<(), sqlx::Error> {
    ...
    pub async fn update_user(self, pool: &Pool) -> Result<Self, sqlx::Error> {
